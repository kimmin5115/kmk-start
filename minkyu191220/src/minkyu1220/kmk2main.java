		...
		while(run) {
			...
			if(kmk == 1) {
				...
				for(i=0; i<student.size(); i++) {
					if(name.equals(student.get(i).getName())) {
						...
						student.get(i).setJavascore(javascore);
						student.get(i).setServletscore(servletscore);
						student.get(i).setSpringscore(springscore);
					}
			...
			//4. 과목별 최고점수
				...
				if( k == 1) {
					for(i=0; i<student.size(); i++) {
						if(max<student.get(i).getJavascore()) {
							max = student.get(i).getJavascore();
							 kname = student.get(i).getName();
						}
						
					}System.out.println("java의 최고 점수는"+max+"입니다.");
				}if( k == 2) {
					for(i=0; i<student.size(); i++) {
						if(max<student.get(i).getServletscore()) {
							max = student.get(i).getServletscore();
							 kname = student.get(i).getName();
						}
					...
					for(i=0; i<student.size(); i++) {
						if(max<student.get(i).getSpringscore()) {
							max = student.get(i).getSpringscore();
							 kname = student.get(i).getName();
						}
